...
export async function generateMetadata({
  ...
  const firm = getFirmByKey(params.key);
  if (!firm) return {};
  ...
  const url = `https://yourdomain.com/firm/${firm.key}`; // TODO: swap base URL
...
export default function FirmDetailPage({ params }: { params: { key: string } }) {
  const firm = getFirmByKey(params.key);
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      url: firm.homepage,
      priceCurrency: "USD",
      // Using payout as a placeholder 'price' is a stretch; omit if you prefer:
      // price: firm.minDays ?? 0,
    ...
    <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-3">
      ...
      <article className="lg:col-span-2">
        <header className="flex items-start gap-4">
          {firm.logo && (
<Logo src={firm.logo ?? "/logos/placeholder.png"} alt={firm.name} size={64} />
          )}
          <div>
...
<div className="mt-6">
  ...
  >
    <Button
      className="w-full border-2 border-transparent hover:border-[#5fffc2] hover:shadow-[0_0_12px_#5fffc2aa] transition-all duration-300"
    >
      Get Started
    </Button>
  </a>
        ...
        {/* Pros/cons style bullets based on booleans */}
